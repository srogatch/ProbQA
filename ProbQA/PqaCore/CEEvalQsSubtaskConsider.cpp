...
#undef EASY_SET
    ...
    volatile const double stableET = ((nExpectedTargets <= epsET) ? epsET : nExpectedTargets);
    ...
    volatile const double stableDist = ((avgD <= epsD) ? epsD : avgD);

    volatile const double priority = std::expm1(cExpMul * stableDist) / stableET;
