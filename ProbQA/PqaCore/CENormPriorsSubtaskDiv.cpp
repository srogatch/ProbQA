...
template<typename taNumber> CENormPriorsSubtaskDiv<taNumber>::CENormPriorsSubtaskDiv(CENormPriorsTask<taNumber> *pTask)
...
template<> void CENormPriorsSubtaskDiv<SRDoubleNumber>::Run() {
  ...
  __m256d* pMants = SRCast::Ptr<__m256d>(task._pQuiz->GetTlhMants());
